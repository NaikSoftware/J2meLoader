...
public class LocalDevice implements DiscoveryAgent, ActivityResultListener {
	protected static final int REQUEST_CODE_DISCOVERABLE = ContextHolder
			.getRequestCode("SetDiscoverable");
		...
		if (instance == null) {
			...
			if (instance.adapter == null) {
				throw new BluetoothStateException(
						"this system does not have a bluetooth");
			}
		}
	...
	public boolean setDiscoverable(int mode) {
		if (mode != NOT_DISCOVERABLE && getDiscoverable() == NOT_DISCOVERABLE) {
			...
			synchronized (waiter) {
				ContextHolder.getActivity().startActivityForResult(
						discoverableIntent, REQUEST_CODE_DISCOVERABLE);
		...
		BroadcastReceiver receiver = new BroadcastReceiver() {
			public void onReceive(Context context, Intent intent) {
				...
				if (BluetoothDevice.ACTION_FOUND.equals(action)) {
					...
					listener.deviceDiscovered(new RemoteDevice(device),
							new DeviceClass(device.getBluetoothClass()));
	...
	public int searchServices(final int[] attrSet, final UUID[] uuidSet,
			final RemoteDevice btDev, final DiscoveryListener discListener) {
		...
		Runnable runnable = new Runnable() {
			public void run() {
				...
				discListener.serviceSearchCompleted(transID,
